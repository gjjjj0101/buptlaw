...
import { CASE_REASON_MAP, CASE_TYPE_MAP } from "../consts/case";

const caseList = ref<(Case & {
  formatOpenTime: string,
  formatCaseReason: string,
  formatCaseType: string,
})[]>([])
...
getCaseList(1, 10).then(res => {
  caseList.value = res.content.map(item => {
    return {
      ...
      formatCaseReason: CASE_REASON_MAP[item.caseReason as keyof typeof CASE_REASON_MAP],
      formatCaseType: CASE_TYPE_MAP[item.caseType as keyof typeof CASE_TYPE_MAP]
    }
  })
})
  ...
  <div id="case-analysis">
    ...
    <div id="case-analysis-list-box" class="card">
      <el-table :data="caseList" stripe>
        ...
        <el-table-column prop="caseDescription" label="案情概要" width="320"/>
        <el-table-column prop="formatCaseReason" label="案由" width="180"/>
        <el-table-column prop="formatCaseType" label="案件类型" width="180"/>
